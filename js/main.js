...
/* ---------- Init data (categories first, then books) ---------- */
async function initGlobalData() {
    const getCategories = () =>
        JSON.parse(localStorage.getItem('bs_categories') || '[]')
    const saveCategories = (c) =>
        localStorage.setItem('bs_categories', JSON.stringify(c))
    ...
    // 1. Init categories FIRST từ categories.json
    if (existingCategories.length === 0) {
        try {
            const response = await fetch('data/categories.json')
            const mockCategories = await response.json()
            saveCategories(mockCategories)
            console.log(
                '✅ Đã khởi tạo categories từ categories.json:',
                mockCategories
            )
        } catch (error) {
            console.error('❌ Lỗi khi init categories:', error)
        }
    }

    // 2. Init books AFTER categories từ books.json
    if (existingBooks.length === 0) {
        try {
            const response = await fetch('data/books.json')
            const mockBooks = await response.json()

            const convertedBooks = mockBooks.map((book, index) => ({
                id: book.id, // Dùng UUID có sẵn từ JSON
                code: `SP${String(index + 1).padStart(3, '0')}`, // Code dạng SP001, SP002...
                title: book.title,
                author: book.author,
                price: book.price,
                category: book.category,
                stock: Math.floor(Math.random() * 50) + 5,
                image: book.cover || 'https://via.placeholder.com/100x150',
                description: book.desc || '',
                status: 'active',
            }))
            saveBooks(convertedBooks)
            console.log('✅ Đã khởi tạo books từ books.json:', convertedBooks)
        } catch (error) {
            console.error('❌ Lỗi khi init books:', error)
        }
    }
}
...
function renderCart() {
  ...
  `
    ...
    $('#checkout').addEventListener('click', () => {
        ...
        if (!auth) {
            if (
                confirm('Bạn cần đăng nhập để thanh toán. Đến trang đăng nhập?')
            )
...
function initAuthPage() {
    ...
    $('#btn-register').addEventListener('click', () => {
        ...
        if (!email || !pass)
            return ($('#auth-msg').textContent = 'Vui lòng nhập đầy đủ.')
        const users = getUsers()
        if (users.find((u) => u.username === email))
            return ($('#auth-msg').textContent = 'Tài khoản đã tồn tại.')
    ...
    $('#btn-login').addEventListener('click', async () => {
        ...
        // Validate input
        if (!email || !pass) {
            $('#auth-msg').textContent = 'Vui lòng nhập đầy đủ thông tin.'
            return
        }

        // Load users from JSON if localStorage is empty
        let users = getUsers()
        if (users.length === 0) {
            try {
                ...
                console.log('Không thể load users từ file JSON:', error)
            }
        }
        ...
        // Kiểm tra tài khoản
        if (!u) {
            $('#auth-msg').textContent = 'Sai thông tin đăng nhập.'
            return
        }

        // Check tài khoản khóa - user đăng nhập không được
        if (u.locked) {
            $('#auth-msg').textContent =
                'Tài khoản của bạn đã bị khóa. Vui lòng liên hệ admin.'
            $('#auth-msg').style.color = '#ff4444'
            return
        }
...
async function init() {
    ...
    // Init categories & books from JSON (categories first!)
